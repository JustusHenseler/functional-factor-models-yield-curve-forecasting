...
loadings <- t(sapply(workinggrid, function(i) {
  c(1, slope(i), curvature(i))
}))

factors <- matrix(nrow=dim(rawdata)[1], ncol = 3)
for (t in 1:dim(rawdata)[1]){
  factors[t,] <- lm(rawdata[t,] ~ loadings[match(observationgrid, workinggrid),] - 1)$coefficients
}
factors <- ts(factors, start = time(densedata)[1], end = rev(time(densedata))[1], frequency = 12)
...
curve.epsilon <- t(loadings %*% t(factors)) - densedata
...
curve.fitted <- factors.fitted %*% t(loadings)
...
for (i in 1:max.h){
  predicted.curve[i,] <- as.numeric(predicted.factors[i,]) %*% t(loadings)
...
args <- list('p' = p, 'AR' = AR, 'macro' = macro, 'GARCH' = GARCH, 'start' = start, 'end' = end, 'max.h' = max.h)
